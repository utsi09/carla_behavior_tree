...
class CheckRiskLevel3 : public BT::ConditionNode
{
    public:
        CheckRiskLevel3(const std::string& name, const BT::NodeConfiguration& config) : BT::ConditionNode(name, config)
        {
    ...
    NodeStatus tick() override
    {
        ...
        if (last_risk_ >= 30) {
            std::cout<<"위험도 위험 수준 : level " << last_risk_ <<std::endl;
            return BT::NodeStatus::SUCCESS;
        }
        else {
            std::cout<<"위험도 위험 수준 이탈 "<< std::endl;
